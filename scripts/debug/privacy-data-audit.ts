import fs from "node:fs";
import path from "node:path";

const rows = [
  {
    dataName: "食べた記録",
    storage: "localStorage: uniba-food-logs-v1",
    personalData: "メモ、写真URL、支払金額の入力内容次第で該当可能性あり。ただし通常利用では運営者へ自動送信しません。",
    purpose: "食べた履歴、現在販売中コンプ率、図鑑コンプ率、総消費金額",
    deletion: "設定画面または食べたページの全データ削除、バックアップ復元、ブラウザデータ削除",
    appStoreDisclosure: "端末内のみなら収集なし。ユーザーが問い合わせ等で送信した場合のみUser Content",
    googlePlayDisclosure: "端末内のみなら収集なし。ユーザーが問い合わせ等で送信した場合のみUser-provided content"
  },
  {
    dataName: "レビュー・星評価",
    storage: "localStorage: uniba-food-reviews-v1",
    personalData: "コメント内容次第で該当可能性あり。ただし通常利用では運営者へ自動送信しません。",
    purpose: "商品レビュー、平均評価、本人レビュー管理",
    deletion: "設定画面または食べたページの全データ削除、ブラウザデータ削除",
    appStoreDisclosure: "端末内のみなら収集なし",
    googlePlayDisclosure: "端末内のみなら収集なし"
  },
  {
    dataName: "レビュー連投制限",
    storage: "localStorage: uniba-food-review-last-submit-v1",
    personalData: "いいえ",
    purpose: "同一端末の短時間連投防止",
    deletion: "設定画面またはブラウザデータ削除",
    appStoreDisclosure: "収集なし",
    googlePlayDisclosure: "収集なし"
  },
  {
    dataName: "最近見た商品",
    storage: "localStorage: uniba-recent-foods-v1",
    personalData: "いいえ",
    purpose: "ホームの最近見た商品表示",
    deletion: "設定画面またはブラウザデータ削除",
    appStoreDisclosure: "収集なし",
    googlePlayDisclosure: "収集なし"
  },
  {
    dataName: "検索履歴",
    storage: "localStorage: uniba-recent-searches-v1",
    personalData: "検索内容次第で該当可能性あり。ただし端末内のみ",
    purpose: "検索候補表示",
    deletion: "設定画面またはブラウザデータ削除",
    appStoreDisclosure: "収集なし",
    googlePlayDisclosure: "収集なし"
  },
  {
    dataName: "PWA案内状態",
    storage: "localStorage: uniba-pwa-ios-hint-dismissed",
    personalData: "いいえ",
    purpose: "同じ案内を繰り返さない",
    deletion: "ブラウザデータ削除",
    appStoreDisclosure: "収集なし",
    googlePlayDisclosure: "収集なし"
  },
  {
    dataName: "発見報告",
    storage: "Googleフォーム（NEXT_PUBLIC_REQUEST_FORM_URL）",
    personalData: "任意連絡先を入力した場合は該当",
    purpose: "商品追加、価格修正、販売終了報告、情報修正の管理者確認",
    deletion: "お問い合わせから削除依頼。管理者がGoogleフォーム回答を確認して対応",
    appStoreDisclosure: "Contact Info / User Content / Customer Support",
    googlePlayDisclosure: "Personal info / User-provided content"
  },
  {
    dataName: "お問い合わせ",
    storage: "scripts/output/contact-submissions.generated.json",
    personalData: "任意連絡先を入力した場合は該当",
    purpose: "問い合わせ対応、削除依頼、セキュリティ連絡",
    deletion: "お問い合わせから削除依頼",
    appStoreDisclosure: "Contact Info / Customer Support",
    googlePlayDisclosure: "Personal info / App activity"
  },
  {
    dataName: "管理者アクセスキー",
    storage: "Cookie: admin_key / query parameter",
    personalData: "いいえ",
    purpose: "本番外部アクセス時の管理画面保護",
    deletion: "Cookie削除",
    appStoreDisclosure: "収集なし",
    googlePlayDisclosure: "収集なし"
  },
  {
    dataName: "エラー監視",
    storage: "NEXT_PUBLIC_SENTRY_DSN設定時にSentryへ送信",
    personalData: "通常いいえ。エラーに入力値を含めない運用方針",
    purpose: "障害調査、品質改善",
    deletion: "Sentry側保持期間設定、削除依頼",
    appStoreDisclosure: "Diagnostics",
    googlePlayDisclosure: "App info and performance"
  },
  {
    dataName: "アクセス解析",
    storage: "NEXT_PUBLIC_ANALYTICS_ENDPOINT設定時に指定endpointへ送信 / localStorage: uniba-local-analytics-v1は開発設定時のみ",
    personalData: "通常いいえ",
    purpose: "ページ表示傾向、改善優先度判断",
    deletion: "endpoint側削除、設定解除、ブラウザデータ削除",
    appStoreDisclosure: "Usage Data if sent off-device; local onlyなら収集なし",
    googlePlayDisclosure: "App activity if sent off-device; local onlyなら収集なし"
  },
  {
    dataName: "Cookie / sessionStorage / IndexedDB",
    storage: "標準機能では未使用。ログインCookieや認証用sessionStorageは使用しません。",
    personalData: "いいえ",
    purpose: "通常機能では使用しません",
    deletion: "ブラウザデータ削除",
    appStoreDisclosure: "収集なし",
    googlePlayDisclosure: "収集なし"
  }
];

const outputPath = path.join(process.cwd(), "scripts", "output", "privacy-data-audit.generated.json");
fs.mkdirSync(path.dirname(outputPath), { recursive: true });
fs.writeFileSync(outputPath, `${JSON.stringify({ generatedAt: new Date().toISOString(), rows }, null, 2)}\n`);
console.log(JSON.stringify({ outputPath, rows: rows.length }, null, 2));
